...
* **Comprehensive Algorithm Coverage**: 380+ algorithm implementations
* **Post-Quantum Cryptography**: ML-KEM, ML-DSA, SLH-DSA, LMS/HSS, and hybrid key exchange (OpenSSL 3.5+)
* **Modern Algorithms**: AES-GCM-SIV (nonce-misuse resistant), deterministic ECDSA (RFC 6979)
* **Drop-in Replacement**: Standard JCA provider interface

== Requirements

* **Java**: 25 or later (with `--enable-native-access` flag)
* **OpenSSL**: 3.0 or later (`libcrypto.so.3` or `libcrypto.so.4`)
...
|3.2.x |Supported |Adds Argon2 KDFs, AES-GCM-SIV
|3.3.x |Supported |Full support
|3.5.x |Supported |Adds Post-Quantum Cryptography (ML-KEM, ML-DSA, SLH-DSA)
|3.6.x |Supported |Adds LMS/HSS signatures, deterministic ECDSA (RFC 6979)
|4.0.x |Supported |Full support (`libcrypto.so.4`)
|===
