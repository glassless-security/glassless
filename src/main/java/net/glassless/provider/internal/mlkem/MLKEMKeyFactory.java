 ...
 */
public class MLKEMKeyFactory extends AbstractKeyFactory {
   ...
   protected boolean isOwnKey(Key key) {
      return key instanceof GlaSSLessMLKEMPublicKey || key instanceof GlaSSLessMLKEMPrivateKey;
   }
   ...
   /**
    * Detects the ML-KEM variant from an encoded key by scanning for the OID.
    * ML-KEM-512 OID: 2.16.840.1.101.3.4.4.1
    * ML-KEM-768 OID: 2.16.840.1.101.3.4.4.2
    * ML-KEM-1024 OID: 2.16.840.1.101.3.4.4.3
    */
   private String detectAlgorithm(byte[] encoded, boolean isPublic) {
      ...
      return detectAlgorithmFromSize(encoded.length, isPublic);
   }
